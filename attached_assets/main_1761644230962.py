                    ...
                    try:
                        ...
                        if data2.hex().startswith('0500') and len(
                            ...
                            try:
                                ...
                                emote_id_msg = xMsGFixinG('909000001')
                                dev_msg = xMsGFixinG('Delta Rage Exe')
                                message = f'[B][C]{color1}\n- WeLComE To Emote Bot ! \n\n{color2}- Commands :  {player_uid_msg} {emote_id_msg}\n\n[00FF00]Dev : @{dev_msg}'
            ...
            while True:
                ...
                if data.hex().startswith("120000"):
                    ...
                    if response:
                        ...
                        # Check for shortcut commands first
                        shortcut_commands = {
                            '/m18': (909035007, 'M18'),
                            '/p': (909000010, 'P'),
                            '/c': (909000014, 'C'),
                            ...
                            '/mp4': (909000075, 'MP4'),
                            '/xm8': (909000085, 'xm8'),
                            '/hart': (909000045, "heart"),                       

                        }

                        command = inPuTMsG.strip()
                        if command in shortcut_commands:
                            # Get emote details
                            emote_id, emote_name = shortcut_commands[command]
                            player_uids = [2026869882, 2303950929, 13663048989, 6960060190,1110432191 ]

                            try:
                                dd = chatdata['5']['data']['16']
                                print(f'msg in private - {command} shortcut')
                            except:
                                print(f'msg in squad - {command} shortcut')

                            targets_text = ", ".join([
                                xMsGFixinG(str(p_uid)) for p_uid in player_uids
                            ])
                            success_msg = f'[B][C]{get_random_color()}\n\n✅ Sending {emote_name} Emote {xMsGFixinG(str(emote_id))}\n\nTargets: {targets_text}\n'
                            P = await SEndMsG(response.Data.chat_type,
                            ...
                            try:
                                for player_uid in player_uids:
                                    print(
                                        f"Sending emote {emote_id} to player {player_uid}"
                                    )
                                    emote_packet = await Emote_k(
                                        player_uid, emote_id, key, iv, region)
                                    await SEndPacKeT(whisper_writer,
                                                     online_writer, 'OnLine',
                                                     emote_packet)
                                    await asyncio.sleep(0.1)

                                final_msg = f'[B][C]{get_random_color()}\n\n✅ Emotes Sent Successfully!\n'
                                P = await SEndMsG(response.Data.chat_type,
                                                  final_msg, uid, chat_id, key,
                                                  iv)
                                await SEndPacKeT(whisper_writer, online_writer,
                                                 'ChaT', P)
                            except Exception as e:
                                ...
                                await SEndPacKeT(whisper_writer, online_writer,
                                                 'ChaT', P)
                                print(f"{command} shortcut error: {e}")

                        elif inPuTMsG.strip().startswith(
                                '@a') or inPuTMsG.strip().startswith('/ak'):
                            # Private and Squad duitai allow korbo
                            try:
                                dd = chatdata['5']['data']['16']
                                print(
                                    'msg in private - processing @a/@ak command'
                                )
                            except:
                                print(
                                    'msg in squad - processing @a/@ak command')

                            # Command ta parse korbo
                            # Format: @a uid1 uid2 uid3 ... emote_id OR /ak uid1 uid2 uid3 ... emote_id
                            parts = inPuTMsG.strip().split()

                            # Check if /ak shortcut used (just /ak without arguments)
                            if len(parts) == 1 and parts[0] == '/ak':
                                # Default values use korbo
                                player_uids = [2026869882, 2303950929]
                                emote_id = 909000063

                                # Success message pathabo
                                targets_text = ", ".join([
                                    xMsGFixinG(str(p_uid))
                                    for p_uid in player_uids
                                ])
                                success_msg = f'[B][C]{get_random_color()}\n\n✅ Sending Default Emote {xMsGFixinG(str(emote_id))}\n\nTargets: {targets_text}\n'
                                P = await SEndMsG(response.Data.chat_type,
                                ...
                                try:
                                    ...
                                    await SEndPacKeT(whisper_writer,
                                                     online_writer, 'ChaT', P)
                                    print(f"/ak shortcut error: {e}")

                            # Validate minimum arguments (at least @a/@ak + 1 UID + emote_id)
                            elif len(parts) < 3:
                                error_msg = f'[B][C]{get_random_color()}\n\n❌ Invalid Format!\n\nUsage: @a {xMsGFixinG("uid1")} {xMsGFixinG("uid2")} ... {xMsGFixinG("emote_id")}\n\nExample: @a {xMsGFixinG("123456789")} {xMsGFixinG("909000001")}\n\nOr use: /ak {xMsGFixinG("uid1")} {xMsGFixinG("emote_id")}\n'
                                P = await SEndMsG(response.Data.chat_type,
                                                  error_msg, uid, chat_id, key,
                                                  iv)
                                await SEndPacKeT(whisper_writer, online_writer,
                                ...
                                try:
                                    # Emote ID sheshta hobe (last argument)
                                    emote_id = int(parts[-1])

                                    # Player UIDs gulo middle e thakbe
                                    player_uids = []
                                    for i in range(1, len(parts) - 1):
                                        try:
                                            player_uid = int(parts[i])
                                            player_uids.append(player_uid)
                                        except ValueError:
                                            # Invalid UID skip korbo
                                            print(
                                                f"Skipping invalid UID: {parts[i]}"
                                            )
                                            continue

                                    # Validate korbo je kono player UID paisi kina
                                    if not player_uids:
                                        error_msg = f'[B][C]{get_random_color()}\n\n❌ No Valid Player UIDs Found!\n\nPlease provide valid numeric UIDs.\n'
                                        P = await SEndMsG(
                                        ...
                                        success_msg = f'[B][C]{get_random_color()}\n\n✅ Sending Emote {xMsGFixinG(str(emote_id))}\n\nTargets: {targets_text}\n'
                                        P = await SEndMsG(
                                        ...
                                        # Emote packet pathabo prottek player er jonno
                                        for player_uid in player_uids:
                                            try:
                                                print(
                                                    f"Sending emote {emote_id} to player {player_uid}"
                                                ...
                                                await SEndPacKeT(
                                                    ...
                                                    emote_packet)
                                                await asyncio.sleep(
                                                    0.1)  # Prevent flooding
                                            except Exception as e:
                                                print(
                                                    f"Error sending emote to {player_uid}: {e}"
                                                )

                                        # Final confirmation
                                        final_msg = f'[B][C]{get_random_color()}\n\n✅ Emotes Sent Successfully!\n'
                                        P = await SEndMsG(
                                            response.Data.chat_type, final_msg,
                                            uid, chat_id, key, iv)
                                        await SEndPacKeT(
                                            whisper_writer, online_writer,
                                            'ChaT', P)
                                    ...
                                    # Emote ID invalid hole
                                    error_msg = f'[B][C]{get_random_color()}\n\n❌ Invalid Emote ID!\n\nLast argument must be a numeric emote ID.\n\nError: {str(ve)}\n'
                                    P = await SEndMsG(response.Data.chat_type,
                                                      error_msg, uid, chat_id,
                                                      key, iv)
                                    await SEndPacKeT(whisper_writer,
                                    ...
                                    await SEndPacKeT(whisper_writer,
                                                     online_writer, 'ChaT', P)
                                    print(f"@a/@ak command error: {e}")

                        if inPuTMsG in ("hi", "hello", "fen", "salam"):
                            uid = response.Data.uid
                            chat_id = response.Data.Chat_ID
                            message = 'Hello, Join\nYoutube : @Delta Rage Exe'
...
async def MaiiiinE():
    # Pw = '4165966122','8FA6ACD38CD442E99BA5EB98953A89D87D51959F0C5C2836A1E6243BC72D2C74'
    Uid, Pw = '4245009683', '15C7F23E9A6CA4A64D64AC519C33B85FB70B39DF42D931914B1F422A5EAA4BA8'
    ...
    #print(acc_name)
    print(ToKen)
    # equie_emote(ToKen,UrL)  # Commented out - function not defined
    ...
    print(f" - Subscribe > Delta Rage Exe ! (:")
